...
FileManager.prototype.getFileList = function (pattern, callback) {
    var self = this;
    // options is optional -> what options
    glob(self.fileManagerPath + pattern, function (er, files) {
        var cleanFiles = [];
        for (var fileFilter in files) {
            if (files.hasOwnProperty(fileFilter)) { // this is redundant, for in
                // only iterates over enumerable properties that exist,  
                // i.e. files.hasOwnProperty(fileFilter) is always true...
                cleanFiles.push(files[fileFilter].replace(self.fileManagerPath, ""));
            }
        }
        // how come you return callback in createFolderIfNotExist() method
        // but not here?
